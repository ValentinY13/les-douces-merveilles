    ...
    <section aria-labelledby="Détail du produit">
      ...
      <div class="px-6 lg:pt-14">
        <div class="responsive-layout lg:grid lg:grid-cols-2 gap-x-16">
          <BackButton class="hidden lg:block lg:col-span-2 mb-6 reveal reveal-left-300"/>

          <ProductWithThumbnail class="hidden lg:row-span-2" :images="product.images"/>

          <div class="space-y-6 py-12 lg:pt-0">
            <h1 id="Détail du produit" class="text-h2 text-brown-700 uppercase reveal reveal-left-300">{{
                product.name
              }}</h1>
            <p class="text-brown-100 tracking-wider reveal reveal-left-400">{{ product.number_pieces }}
            ...
            <div class="flex flex-col md:flex-row lg:flex-col gap-6">
              <AddRemove :is-available="isAvailable" :product-name="product.name" :max="product.max"
                         v-model:quantity="quantity" class="reveal reveal-left-500"/>
              <div class="reveal reveal-left-600">
                <button
          ...
          <div class="pb-12 lg:col-start-2 lg:row-start-3 reveal reveal-bottom-400">
      ...
      <LayoutCarouselSection>
        ...
        <swiper-slide v-for="(product, i) in sameCatProducts" :key="product.id"
                      class="max-w-[450px] w-full h-auto" :class="`reveal reveal-bottom-${i}00`">
